...
class BD_Pressure_Advance:
    ...
    def cmd_SET_BDPRESSURE(self, gcmd):
        ...
        if 'START' in cmd:
            ...
            self.cmd_reset_probe(gcmd)  # 现在能找到该函数了
        elif 'READ' in cmd:
            self.cmd_read(gcmd)

    # ========== 新增：补全 cmd_reset_probe 函数 ==========
        ...
        if self.usb and "usb" == self.port:
            try:
                ...
                if self.usb.in_waiting:
                    res = self.usb.read(self.usb.in_waiting).decode('utf-8', errors='ignore').strip()
                    self.gcode.respond_info(f"重置传感器成功: {res}")
            except Exception as e:
                logging.error(f"串口重置指令失败: {e}")
                self.gcode.respond_info(f"重置传感器失败: {str(e)}")
        elif "i2c" == self.port:
            try:
                self.write_register('reset_probe', 1)
                self.gcode.respond_info("I2C传感器重置成功")
            except Exception as e:
    ...
    def cmd_stop(self, gcmd):
        ...
        if len(self.PA_data) >= 5:
            ...
            # 过滤无效数据（Hk/Ha为0的情况）
            valid_data = [d for d in self.PA_data if d[4] > 0 and d[5] > 0]
            if not valid_data:
